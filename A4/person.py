...
CSE 415
Assignment 4
Wicked Problem: Dyadic Relationships between Humans

Runs on Astar, BFS, DFS
unlikely to find a solution path given complexity and randomness
"""
import random, math
...
# example goal state
CREATE_GOAL_STATE = lambda: State({'interest': 0.75, 'satisfaction': 3, 'frequency': 7})

CREATE_INITIAL_STATE = lambda: State({'interest': 0.5, 'satisfaction': 3, 'frequency': 2})


def goal_test(s, tol=0.1):
...
def h(state):
    ...
    g = CREATE_GOAL_STATE()
    return math.sqrt((g.d['interest'] - state.d['interest'])**2 + (g.d['satisfaction'] - state.d['satisfaction'])**2)
...
first we have a random variable a (mood) from 0.01 to 1,
when doing 'pos' operator, a = a + 0.2,
and when doing 'neg' operator, a = a - 0.2.
For either operator,
new interest = old interest + V,
where V = frequency / (10 * satisfaction) * interest * (2 * a - 1) - 0.01
